...
更新：2026-09-10。依据本次 v1.17.2 产品审查。前五轮改动已发布为 v1.18.0 并部署至 andnode。后续首屏 KPI 外露与去重优化已发布为 v1.18.1 并完成 andnode 生产部署与服务端/UI 验收。
...
- [x] 移除限时撤销（2026-09-10 用户决定，本地待发布）。保留历史操作记录，自然语言统一走普通处理流程，不设撤销词语拦截。
...
- 按用户决定移除聊天撤销入口和逆向操作实现，清理当前功能介绍；历史发布记录保留。
- 普通删除不再为撤销额外读取远端快照。既有数据库字段与操作记录继续保留，不做破坏性迁移。
- [x] 标题明确但未匹配时返回未找到；空标题不作为通配候选，避免误改或误删其他日程。
- [x] 日程定位按最新成功操作判断有效状态；删除失败保留可定位日程，成功删除才隐藏，后续恢复重新生效，同时间戳按记录 ID 排序。删除失败不再提示已删除，本地模式删除正常更新状态。
- [x] 聊天重试优先解析引用消息；无引用时在单条和批次之间选择最近失败操作，拒绝未知引用和已有后续操作的旧记录。
- [x] 引用后的 AI 结果必须包含有效的修改/删除意图；错误、缺字段、不支持、无修改内容和内容未变时不执行日历写入。
- 用户进一步明确不特殊拦截“撤销”等词语，统一按普通消息、引用修改、草稿补充等既有流程处理；专用撤销执行器保持移除。
- 全量测试 505 passed、35 warnings；新增 36 个四项修复回归场景，保留三渠道自然语言路由、消息绑定、批次隔离等测试。git diff --check 通过。用户已授权测试通过后发布部署，线上状态以实际验收为准。
